	...
	if data and data[index] then
		if saveData then
			DataStore.updateDataAsync(dataStore, index, function(currentData, keyInfo)
				if typeof(currentData) == "table" and typeof(data[index]) == "table" and Table.deepCheckEquality(currentData, data[index]) then
					warn("Data identical, not updating")
					return -- If the data is identical, return
				end
	...
	for dataStore, data in pairs(DataStore.DataCache) do
		for index, value in pairs(data) do
			--[[if typeof(value) == "number" then
				...
				DataStore.updateDataAsync(dataStore, index, function(currentData, keyInfo)
					if typeof(currentData) == "table" and typeof(value) == "table" and Table.deepCheckEquality(currentData, value) then
						warn("Data identical, not updating")
						return -- If the data is identical, return
					end
					local userIDs = keyInfo:GetUserIds()
